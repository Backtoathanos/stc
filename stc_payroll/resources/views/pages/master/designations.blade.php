...
<div class="card">
  <div class="card-header">
    <h3 class="card-title">Designations Management</h3>
  ...
  <div class="card-body">
    ...
    <div class="table-responsive">
      <table id="designationsTable" class="table table-bordered table-striped">
        <thead>
          <tr>
            <th>ID</th>
            <th>Name</th>
...
<!-- View Modal -->
<div class="modal fade" id="viewDesignationModal" tabindex="-1" role="dialog">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">View Designation Details</h5>
        <button type="button" class="close" data-dismiss="modal">
          <span>&times;</span>
        </button>
      </div>
      <div class="modal-body" id="viewDesignationContent">
    ...
    <div class="modal-content">
      ...
      <form id="addDesignationForm">
        <div class="modal-body">
          <div class="form-group">
            <label>Name <span class="text-danger">*</span></label>
            <input type="text" class="form-control" id="addDesignationName" name="name" required>
          </div>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
...
$(document).ready(function() {
    var table = $('#designationsTable').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: "/stc/stc_payroll/public/master/designations/list",
            type: 'GET'
        },
        columns: [
            { data: 'id', name: 'id' },
    ...
    $(document).on('click', '.view-btn', function() {
        var id = $(this).data('id');
        $.ajax({
            url: "/stc/stc_payroll/public/master/designations/show/" + id,
            type: 'GET',
            success: function(response) {
                if (response.success) {
                    var designation = response.data;
                    var html = '<div class="row">' +
                        '<div class="col-md-6 mb-3"><strong>ID:</strong><br><span class="text-muted">' + (designation.id || 'N/A') + '</span></div>' +
                        '<div class="col-md-6 mb-3"><strong>Name:</strong><br><span class="text-muted">' + (designation.name || 'N/A') + '</span></div>' +
                        '<div class="col-md-6 mb-3"><strong>Created At:</strong><br><span class="text-muted">' + (designation.created_at || 'N/A') + '</span></div>' +
                        '<div class="col-md-6 mb-3"><strong>Updated At:</strong><br><span class="text-muted">' + (designation.updated_at || 'N/A') + '</span></div>' +
                        '</div>';
                    $('#viewDesignationContent').html(html);
    ...
    $(document).on('click', '.edit-btn', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    var designation = response.data;
                    $('#editDesignationId').val(designation.id);
                    $('#editDesignationName').val(designation.name || '');
    ...
    $('#editDesignationForm').on('submit', function(e) {
        e.preventDefault();
        var id = $('#editDesignationId').val();
        var formData = $(this).serialize();
        
        $.ajax({
            url: "/stc/stc_payroll/public/master/designations/" + id,
            type: 'POST',
            ...
            error: function(xhr) {
                var message = 'Failed to update designation';
    ...
    $('#addDesignationForm').on('submit', function(e) {
        ...
        $.ajax({
            url: "/stc/stc_payroll/public/master/designations",
            type: 'POST',
            data: formData,
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    Swal.fire({
                        icon: 'success',
                        title: 'Success!',
                        text: response.message || 'Designation added successfully',
                        timer: 2000,
                        showConfirmButton: false
                    });
                    table.draw();
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: response.message || 'Failed to add designation'
                    });
                }
            },
            error: function(xhr) {
                var message = 'Failed to add designation';
                if (xhr.responseJSON && xhr.responseJSON.message) {
                    message = xhr.responseJSON.message;
                } else if (xhr.responseJSON && xhr.responseJSON.errors) {
                    var errors = xhr.responseJSON.errors;
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    error: function(xhr) {
                        var message = 'Failed to delete designation';
                        if (xhr.responseJSON && xhr.responseJSON.message) {
                            message = xhr.responseJSON.message;
                        }
                        Swal.fire({
