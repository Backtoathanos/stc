                  ...
                  <h3 class="card-title">
                    <?php 
                      ...
                      $count_cust=mysqli_query($con, "
                        SELECT DISTINCT `stc_trading_customer_cont_no` FROM `stc_trading_customer`
                      ");
                      ...
                      foreach($calculate_total as$cal_row){
                        $amount=$cal_row['stc_trading_sale_items_item_qty'] * $cal_row['stc_trading_sale_items_item_price'];
                        $gst=$amount * $cal_row['stc_trading_sale_items_item_gst']/100;
                ...
                <?php
                    $agent_qry ="
                      ...
                      FROM 
                        `stc_trading_sale` 
                      WHERE 
                        YEAR(`stc_trading_sale_date`) BETWEEN '".$newyear."' AND '".$newyear."' 
                      ORDER BY 
                        `stc_trading_sale_order_by` 
                      ASC
                    ";
                    ...
                    foreach ($agent_qryresult as $agent_row) {    
                      $amount_qry=mysqli_query($con, "
                        ...
                        FROM
                            `stc_trading_sale`
                        INNER JOIN 
                            `stc_trading_sale_items` ON `stc_trading_sale_id` = `stc_trading_sale_items_sale_id`
                        INNER JOIN 
                            `stc_trading_product` ON `stc_trading_product_id` = `stc_trading_sale_items_item_id`
                        WHERE
                            `stc_trading_sale_order_by` = '".$agent_row['stc_trading_sale_order_by']."'
                        AND 
                            YEAR(`stc_trading_sale_date`) BETWEEN '".$newyear."' AND '".$newyear."'
                      ");  
                      $total_amount=0;
                      foreach($amount_qry as $amount_row){
                        $amount=$amount_row['stc_trading_sale_items_item_qty'] * $amount_row['stc_trading_sale_items_item_price'];
                        $gst=$amount * $amount_row['stc_trading_sale_items_item_gst']/100;
                        $total_amount+=$amount + $gst;
                      }    
    
                      $agent_name[]  = $agent_row['stc_trading_sale_order_by'];
                      $sales[] = $total_amount;
                    }
