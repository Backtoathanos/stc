...
class aetherProduct extends tesseract{
	...
	public function stc_call_purchase(){
		$check_loki=mysqli_query($this->stc_dbs, "
			...
			FROM
				`stc_trading_product`
			ORDER BY `stc_trading_product_id` DESC LIMIT 0,50
		");
		$odin='';
		...
		if($do_action == 0){
			...
			foreach ($check_loki as $row) {
				$odin.='
					<tr>
       					<td class="text-left">
       					  <p>'.$row["stc_trading_product_title"].'</p>
       					</td>
       					<td class="text-left">
       					  <p>'.$row["stc_trading_product_details"].'</p>
       					</td>
       					<td class="text-center">
       					  <p>'.$row["stc_trading_product_gst"].'%</p>
       					</td>
            		</tr>         	
		    	';				
			}
