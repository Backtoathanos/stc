...
<html lang="en">
  <head>
    ...
    <title>Retail Invoice - STC-<?php echo substr("0000{$get_stc_purchase_product['stc_sale_product_bill_id']}", -$str_length);?></title>
                      ...
                      foreach ($checkpurchaseorderitems as $key => $value) {
                        if(isset($_SESSION["stc_print_prevew"])) {  
                          ...
                          foreach($_SESSION["stc_print_prevew"] as $keys => $values){  
                            if($_SESSION["stc_print_prevew"][$keys]['product_id'] == $value["stc_product_id"]){  
                                 $is_available++;  
                          ...
                          if($is_available < 1) {  
                            $item_array = array(  
                                 ...
                                 'product_name'             =>     $value["stc_product_name"],
                  ...
                  foreach($sepvalqry as $comrow){
                    if($comrow['stc_sale_product_dc_keys']=="directchallaned"){
                      $nestsepqry=mysqli_query($con, "
                        ...
                        WHERE `stc_sale_product_dc_items_sale_product_id`='".$_GET['pid']."'
                      ");
                      ...
                      $nestsepqry=mysqli_query($con, "
                        ...
                        WHERE `stc_sale_product_vc_items_sale_product_id`='".$_GET['pid']."'
                      ");
                      foreach($nestsepqry as $nestrow){
                        $value=0;
                        if($nestrow['stc_product_gst']==5){
              ...
              <div class="pre-cond-plus-total-taxb">                
                <table border="0" cellspacing="0" cellpadding="0" id="amount-row-calc">
                  ...
                  <tr class="">
                    ...
                    <td align="left"><b><?php echo round($totalgst + $grtotal + $tcsvalue, 2);?></b></td>
                  </tr>
                </table>
              </div>
            </div>         
