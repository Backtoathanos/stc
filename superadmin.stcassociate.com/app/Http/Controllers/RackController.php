...
class RackController extends Controller
{
    ...
    function list(Request $request){
        ...
        $totalRecordswithFilter = Rack::select('count(*) as allcount')->where('stc_rack_name', 'like', '%' .$searchValue . '%')->count();

        // Fetch records
        $records = Rack::orderBy($columnName,$columnSortOrder)
        ->where('stc_rack.stc_rack_name', 'like', '%' .$searchValue . '%')
        ->select('stc_rack.*')
        ->skip($start)
        ->take($rowperpage)
        ->get();
        ...
        foreach($records as $record){
            $id = $record->stc_rack_id;
            $name = $record->stc_rack_name;
            $status = $record->stc_rack_status==1 ? 'Active' : 'In-active';
            
            $stc_rack_id = '<span id="display-'.$id.'">'.$id.'</span>';
            $stc_rack_name = '<span id="display-name'.$id.'">'.$name.'</span>';
            $stc_rack_status = '<span id="display-status'.$id.'">'.$status.'</span>';
            $actionData = '
            ...
            '; 
            $data_arr[] = array(
            "stc_rack_id" => $stc_rack_id,
            "stc_rack_name" => $stc_rack_name,
            "stc_rack_status" => $stc_rack_status,
    ...
    function update(Request $request){
        ...
        $edit = Rack::where('stc_rack_id', $request->id)->update($update);
        if($edit){
            $response = [
                'status'=>'ok',
                'success'=>true,
    ...
    function delete(Request $request){
        $delete =  Rack::destroy($request->id);
        if($delete){
            $response = [
                'status'=>'ok',
                'success'=>true,
