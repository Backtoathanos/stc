                          ...
                          <?php
                            ...
                            if(empty($_GET['inv_pd_search'])){
                              ...
                              if(mysqli_num_rows($stcelectroinvquery)>0){
                                foreach($stcelectroinvquery as $invrow){
                                  $loki_findratefrompo=mysqli_query($con, "
                                    ...
                                    WHERE `stc_purchase_product_items_product_id`='".$invrow["stc_product_id"]."' 
                                  ...
                                  ");
                                  $pbasic=0;
                                  foreach($loki_findratefrompo as $stdcpurraterow){
                                    $pbasic+=$stdcpurraterow['stc_purchase_product_items_rate'];
                                  }
                                  $gstamount = ($pbasic * $invrow["stc_product_gst"])/100;
                                  $pamount=$pbasic + $gstamount;
                                  $percalamount = ($pamount * $invrow['stc_product_sale_percentage'])/100;
                                  $stcmazetotal=$pamount + $percalamount;
                                  echo "
                                    <tr>
                                      <td>".$sl."</td>
                                      <td>".$invrow['stc_product_id']."</td>
                                      <td>".$invrow['stc_product_name']."</td>
                                      <td>".$invrow['stc_cat_name']."</td>
                                      <td>".number_format($pamount, 2)."</td>
                                      <td>".$invrow['stc_product_sale_percentage']."%</td>
                                      <td>
                                          ".number_format($invrow['stc_electronics_inventory_item_qty'], 2)."/
                                          ".$invrow['stc_product_unit']."
                                      </td>
                                      <td>".number_format($stcmazetotal, 2)."</td>
                                    </tr>
                                  ";
                                  $sl++;
                                }
                              ...
                              $stcelectroinvquery=mysqli_query($con, "
                                ...
                                INNER JOIN `stc_category`
                                ON `stc_cat_id`=`stc_product_cat_id`
                                WHERE (`stc_product_name` REGEXP '".mysqli_real_escape_string($con, $_GET['inv_pd_search'])."'
                                OR `stc_product_desc` REGEXP '".mysqli_real_escape_string($con, $_GET['inv_pd_search'])."')
