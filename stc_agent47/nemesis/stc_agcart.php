...
class transformers extends tesseract{	
		...
		if(!empty($_SESSION["stc_agent_cart_sess"])){  		
			...
			foreach($_SESSION["stc_agent_cart_sess"] as $cartpditem){
				...
				$order_table.='
					<tr>
						<td>'.$sl.'</td>
						<td>'.$subcat.' '.$bumblebeetakeit['stc_product_name'].'</td>
						<td>
							<input 
								type="text" 
						...
						<td align="right">'.$cartpditem['product_price'].'</td>
						<td align="right">'.number_format($cartpditem['product_quantity'] * $cartpditem['product_price'], 2).'</td>
			...
			$order_table.='
					<tr>
						<td colspan="7" align="center">Cart Empty!!!</td>
					</tr>
			';
		}
		$order_table.='
	...
	public function stc_agent_order_add($sitename, $siteaddress, $cust_id){
		$optimusprime='';
...
if(isset($_POST['order_add'])){
	...
	if(empty($sitename) || empty($siteaddress)){
		$out="Please Fill All Fields!!!";
	}elseif($cust_id=="NA"){
		$out="Please Select Customer First!!!";
	}elseif(empty($_SESSION['stc_agent_cart_sess'])){
		$out="Please Buy Something!!!";
	}else{
		$outbumblembee=$bumblebee->stc_agent_order_add($sitename, $siteaddress, $cust_id);
