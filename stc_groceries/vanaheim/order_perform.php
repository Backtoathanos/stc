...
class aetherPurchase extends tesseract{	
	...
	// Search product name in groceries purchase Page
	public function stc_search_product_name($searchme){
		$purchase_rate='';
		$alagsequery="
			SELECT
				`stc_groceries_product_id`,
			...
			WHERE 
				`stc_groceries_product_title` REGEXP '".mysqli_real_escape_string($this->stc_dbs, $searchme)."' 
				OR 
				`stc_groceries_product_details` REGEXP '".mysqli_real_escape_string($this->stc_dbs, $searchme)."'
			ORDER BY `stc_groceries_product_id` 
			ASC LIMIT 0,20
		";
			            		    ...
			            		    <div class="box-up">
			            		        <div class="img-info">
				             	        <div class="info-inner">
				             	            <span class="p-name">'.htmlentities($row["stc_groceries_product_title"]).'</span>
				             	            <span class="p-company">GST : '.$row["stc_groceries_product_gst"].'% </span>
				             	            <span class="p-qty" >
				             	            	<select class="form-control validate " id="stcpdunit'.$row["stc_groceries_product_id"].'" style="color:black;font-weight:bold;text-align:center">
				             	        	...
				             	        	<input
						                	  	id="stcpdqty'.$row["stc_groceries_product_id"].'"
						                	  	name="stcpdqty"
						                	  	type="number"
			             	         	...
			             	         	<input 
			             	         		type="hidden" 
			             	         		name="hidden_name" 
			             	         		id="stcpdgst'.$row["stc_groceries_product_id"].'" 
			             	         		value="'.htmlentities($row["stc_groceries_product_gst"]).'" 
			             	         	/>
			             	         	<input 
			             	         		type="hidden" 
			             	         		name="hidden_name" 
			             	         		id="stcpdname'.$row["stc_groceries_product_id"].'" 
			             	         		value="'.htmlentities($row["stc_groceries_product_title"]).'" 
			             	         	/>
			             	         	<div class="a-size">
				             	            <input 
					             	          	type="number" 
					             	          	name="stcpdprice" 
					             	          	id="stcpdprice'.$row["stc_groceries_product_id"].'" 
		             				...
		             				<div class="box-down">
		             					...
		             					<a class="cart add_to_silent_purchase_cart" href="#" id="'.$row["stc_groceries_product_id"].'">
	...
	// go data to groceries purchase table
	public function purchase_process($silent_purchase_merchantname, $silent_purchase_refreno, $silent_purchase_redredate, $silent_purchase_remarks){
		...
		if($lokiinsertsale){
			$loki_get_max_id=mysqli_query($this->stc_dbs, "
				SELECT * FROM `stc_groceries_purchase` ORDER BY `stc_groceries_purchase`.`stc_groceries_purchase_id` DESC
			");
			$stc_purchase_product_items_order_id=mysqli_fetch_assoc($loki_get_max_id);
			$get_purchase_product_id=$stc_purchase_product_items_order_id['stc_groceries_purchase_id'];
			if(empty($_SESSION['stc_silent_purchase_sess'])){
				...
				foreach ($_SESSION["stc_silent_purchase_sess"] as $key=>$value) {
			    	$friday_loki=mysqli_query($this->stc_dbs, "
			    		INSERT INTO `stc_groceries_purchase_items`(
							`stc_groceries_purchase_items_purchase_id`,
							`stc_groceries_purchase_items_item_id`,
	...
	public function stc_filter_purchase($search, $begdate, $endDate){
		...
		if($search==''){
			...
			$search="
				(
					`stc_groceries_merchant_title` REGEXP '".mysqli_real_escape_string($this->stc_dbs, $search)."'
					OR `stc_groceries_purchase_refrence_no` REGEXP '".mysqli_real_escape_string($this->stc_dbs, $search)."'
					OR `stc_groceries_purchase_remarks` REGEXP '".mysqli_real_escape_string($this->stc_dbs, $search)."'
				) AND 
			";
		}
		$check_loki=mysqli_query($this->stc_dbs, "
			SELECT * FROM `stc_groceries_purchase` 
			INNER JOIN `stc_groceries_merchant` 
			ON `stc_groceries_merchant_id`=`stc_groceries_purchase_purchaser_id` 
			INNER JOIN `stc_groceries_user` 
			ON `stc_groceries_user_id`=`stc_groceries_purchase_createdby` 
			WHERE ".$search."(
                DATE(`stc_groceries_purchase_date`)
                 BETWEEN '".mysqli_real_escape_string($this->stc_dbs, $begdate)."'
                 AND '".mysqli_real_escape_string($this->stc_dbs, $endDate)."'
            )
			ORDER BY DATE(`stc_groceries_purchase_date`) 
			DESC LIMIT 0,20
		");
		$odin='
		...
		if($do_action == 0){
			...
			foreach ($check_loki as $row) {
				$check_loki_nest=mysqli_query($this->stc_dbs, "
					SELECT * FROM `stc_groceries_purchase_items` 
					WHERE `stc_groceries_purchase_items_purchase_id`='".$row["stc_groceries_purchase_id"]."'
				");
				$soamount=0;
				$sogstamount=0;
				foreach ($check_loki_nest as $row2nd) {
					$amt = $row2nd['stc_groceries_purchase_items_qty'] * $row2nd['stc_groceries_purchase_items_price'];
					$gst = $amt * $row2nd['stc_groceries_purchase_items_gst']/100;
					$soamount += $amt;
					$sogstamount += $gst;
				}
				$str_length = 5;
              	// hardcoded left padding if number < $str_length
              	$purchasenumber = substr("0000{$row["stc_groceries_purchase_id"]}", -$str_length);
              	...
              	foreach($purchase_payqry as $purchase_payrow){
              		$paid_amt+=$purchase_payrow['stc_groceries_purchase_payment_value'];
              	}
				...
				$today = date("Y/m/d"); 
				$startTimeStamp = strtotime($row["stc_groceries_purchase_date"]);
				...
				$editopt='
					<a 
       					...
       					id="'.$row["stc_groceries_purchase_id"].'" 
       					href="#" 
				...
				$odin.='
					<tr>
       					<td>
       						'.date('d-m-y', strtotime($row["stc_groceries_purchase_date"])).'
       						<br>
       						STC/T/P/'.$purchasenumber.'
       					</td>
       					<td class="text-center">
       						'.$row["stc_groceries_merchant_title"].'
       					</td>
       					<td class="text-center">
       						'.date('d-m-y', strtotime($row["stc_groceries_purchase_refrence_date"])).'
	       					<br>
	       					'.$row["stc_groceries_purchase_refrence_no"].'
       					</td>
       					<td class="text-right">
       					...
       					<td>
       						'.$row["stc_groceries_purchase_remarks"].'
       					</td>
       					<td class="text-center">
       						'.strtoupper($row["stc_groceries_user_name"]).'
       					</td>
       					<td class="text-center">
       						'.$editopt.'
       						<a class="stc_print_purchase_items" id="'.$row["stc_groceries_purchase_id"].'" href="#" style="font-size: 25px;color: black;">
       							<i class="fa fa-print" aria-hidden="true"></i>
       						</a> <br>
       						<a class="stc_print_payment" id="'.$row["stc_groceries_purchase_id"].'" pay-val="'.$total.'" href="#" style="font-size: 25px;color: black;">
       							Pay
       						</a> 
       					</td>
            		</tr>         	
	...
	public function stc_filter_purchase_items($purchase_id){
		$odin_getqry=mysqli_query($this->stc_dbs, "
			...
			INNER JOIN `stc_groceries_product` 
			ON `stc_groceries_purchase_items_item_id`=`stc_groceries_product_id`
			WHERE
				`stc_groceries_purchase_items_purchase_id` = '".mysqli_real_escape_string($this->stc_dbs, $purchase_id)."'
			ORDER BY `stc_groceries_product_title` ASC
		");
	    ...
	    if(mysqli_num_rows($odin_getqry)>0){
	    	foreach($odin_getqry as $odin_getqryrow){
	    		$slno++;
	    		$price=$odin_getqryrow['stc_groceries_purchase_items_price'] + ($odin_getqryrow['stc_groceries_purchase_items_price'] * $odin_getqryrow['stc_groceries_purchase_items_gst']/100);
	    		$amount=$price * $odin_getqryrow['stc_groceries_purchase_items_qty'];
	    		$odin.='
	    			<tr>
	    				<td class="text-center">'.$slno.'.</td>
	    				<td class="text-center">'.$odin_getqryrow['stc_groceries_purchase_items_item_id'].'</td>
	    				<td>'.$odin_getqryrow['stc_groceries_product_title'].'</td>
	    				<td class="text-center">'.$odin_getqryrow['stc_groceries_purchase_items_unit'].'</td>
	    				<td class="text-right">'.number_format($odin_getqryrow['stc_groceries_purchase_items_qty'], 2).'</td>
	...
	public function stc_payment_set($purchase_id, $payment_type, $payment_amount, $payment_remarks, $check_amount){
		...
		$odin_checkqry=mysqli_query($this->stc_dbs, "
			...
			FROM 
				`stc_groceries_purchase_payment` 
			WHERE 
				`stc_groceries_purchase_payment_purchase_id` = '".mysqli_real_escape_string($this->stc_dbs, $purchase_id)."'
		");
		foreach($odin_checkqry as $odin_checkrow){
			$paid_amount+=$odin_checkrow['stc_groceries_purchase_payment_value'];
		}
		...
		if($payment_amount>$due_amount){
			...
			$odin_paycheckqry=mysqli_query($this->stc_dbs, "
				INSERT INTO `stc_groceries_purchase_payment`(
					`stc_groceries_purchase_payment_date`,
					`stc_groceries_purchase_payment_purchase_id`,
					`stc_groceries_purchase_payment_type`,
					`stc_groceries_purchase_payment_value`,
					`stc_groceries_purchase_payment_remarks`,
					`stc_groceries_purchase_payment_created_by`
				) VALUES (
					'".mysqli_real_escape_string($this->stc_dbs, $date)."',
	...
	public function stc_sale_call_for_purchaseedit($purchase_id){
		...
		$odincall_saleitemqry=mysqli_query($this->stc_dbs, "
			SELECT
				`stc_groceries_purchase_items_id`,
			    `stc_groceries_purchase_items_purchase_id`,
				`stc_groceries_product_id`,
				`stc_groceries_product_title`,
			    `stc_groceries_purchase_items_qty`,
			...
			INNER JOIN 
				`stc_groceries_product` 
			ON 
				`stc_groceries_product_id`=`stc_groceries_purchase_items_item_id` 
			WHERE
				`stc_groceries_purchase_items_purchase_id` = '".mysqli_real_escape_string($this->stc_dbs, $purchase_id)."'
			ORDER BY `stc_groceries_product_title` DESC
		");
		...
		foreach($odincall_saleitemqry as $values) {  
			$amount=$values["stc_groceries_purchase_items_price"];
			$gstamount = $amount * ($values["stc_groceries_purchase_items_gst"]/100);
			$slno++;
			...
			$purchase_items .= ' 
				<tr>
					...
					<td>'.$values['stc_groceries_product_title'].'</td>
					<td>'.$values["stc_groceries_purchase_items_unit"].'</td>
					<td>
					  <input
					  	style="padding: 5px;height: 40px;"
					    type="text"
					    id="stc-trading-item-qtysc'.$values["stc_groceries_purchase_items_id"].'"
					    class="validate"
					    placeholder="Quantity"
					    value="'.number_format($values["stc_groceries_purchase_items_qty"], 2, '.', '').'"
					  />
					</td>
					<td class="text-right">
					  <input
					  	style="padding: 5px;height: 40px;"
					    type="text"
					    id="stc-trading-item-ratesc'.$values["stc_groceries_purchase_items_id"].'"
					    class="validate"
					    placeholder="Quantity"
					    value="'.number_format($price, 2, '.', '').'"
					  />
					  <input type="hidden" id="stc-trading-item-gstsc'.$values["stc_groceries_purchase_items_id"].'" value="'.$values["stc_groceries_purchase_items_gst"].'">
					</td>
					<td align="right"><i class="fas fa-rupee-sign"></i> '.number_format($price * $values['stc_groceries_purchase_items_qty'], 2).'</td>
					<td align="center">
						<a href="#" class="btn btn-primary stc-customer-update-item-edit" id="'.$values["stc_groceries_purchase_items_id"].'"><i class="fa fa-check-circle"></i></a>
						<a href="#" class="btn btn-danger stc-customer-remove-edit" id="'.$values["stc_groceries_purchase_items_id"].'"><i class="fa fa-trash"></i></a>
					</td>
				</tr>
			';  
			$total += $values["stc_groceries_purchase_items_qty"] * $price;  
			$totalgst +=$gstamount;
		}  
		$purchase_items .= ' 
     		<tr>
	...
	// add item on edit
	public function stc_groceries_sale_item_add($sale_id, $item_id, $item_unit, $item_qty, $item_rate){
		$odin='';
		$odin_checkqry=mysqli_query($this->stc_dbs, "
			SELECT 
				`stc_groceries_purchase_items_id`
			FROM 
				`stc_groceries_purchase_items` 
			WHERE 
				`stc_groceries_purchase_items_id`='".mysqli_real_escape_string($this->stc_dbs, $item_id)."' 
			AND 
				`stc_groceries_purchase_items_purchase_id`='".mysqli_real_escape_string($this->stc_dbs, $sale_id)."' 
		");
		if(mysqli_num_rows($odin_checkqry)>0){
			...
			$odin_callgstqry=mysqli_query($this->stc_dbs, "
            	...
            	FROM 
            		`stc_groceries_product`
            	WHERE 
					`stc_groceries_product_id`='".mysqli_real_escape_string($this->stc_dbs, $item_id)."' 
			");
			foreach($odin_callgstqry as $odin_callgstrow){
				$item_gst=$odin_callgstrow['stc_groceries_product_gst'];
			}
	...
	// update items
	public function stc_groceries_purchase_item_update($item_id, $item_qty, $item_rate, $item_gst){
		$odin='';
		$odin_checkqry=mysqli_query($this->stc_dbs, "
			SELECT 
				`stc_groceries_purchase_items_id` 
		...
		if(mysqli_num_rows($odin_checkqry)>0){
			...
			$odin_updateqry=mysqli_query($this->stc_dbs, "
				UPDATE
				    `stc_groceries_purchase_items`
				SET
					`stc_groceries_purchase_items_qty` = '".mysqli_real_escape_string($this->stc_dbs, $item_qty)."',
					`stc_groceries_purchase_items_price` = '".mysqli_real_escape_string($this->stc_dbs, $saleprice)."'
				WHERE
				    `stc_groceries_purchase_items_id` = '".mysqli_real_escape_string($this->stc_dbs, $item_id)."'
			");
			if($odin_updateqry){
	...
	// remove items
	public function stc_groceries_purchase_item_remove($item_id){
		$odin='';
		$odin_removeqry=mysqli_query($this->stc_dbs, "
			DELETE FROM `stc_groceries_purchase_items` WHERE `stc_groceries_purchase_items_id` = '".mysqli_real_escape_string($this->stc_dbs, $item_id)."'
		");
		if($odin_removeqry){
...
class aetherSale extends tesseract{	
	...
	public function stc_search_product_name($searchme){
		...
		$alagsequery="
			...
			WHERE 
				...
				OR 
				`stc_groceries_product_details` REGEXP '".mysqli_real_escape_string($this->stc_dbs, $searchme)."'
			ORDER BY `stc_groceries_product_title` 
			ASC LIMIT 0,20
		";
		$check_loki=mysqli_query($this->stc_dbs, $alagsequery);
		$odin='';               
			...
			foreach ($check_loki as $row) {
				$prerate_qry=mysqli_query($this->stc_dbs, "
					...
					FROM
						`stc_groceries_purchase_items`
					WHERE
						`stc_groceries_purchase_items_item_id`='".$row['stc_groceries_product_id']."'
					ORDER BY `stc_groceries_purchase_items_id` DESC LIMIT 0,4
				");
				foreach($prerate_qry as $prerate_row){
					$gst=$prerate_row['stc_groceries_purchase_items_price'] * $prerate_row['stc_groceries_purchase_items_gst']/100;
					$rate=$prerate_row['stc_groceries_purchase_items_price'] + $gst;
					$purchase_rate.='<option value="'.number_format($rate, 2).'">'.number_format($rate, 2).'/'.$prerate_row['stc_groceries_purchase_items_unit'].'</option>';
				}
				$odin.='	
						<div class="col-xl-4 col-lg-4 col-md-4 col-sm-4">
		             	  	<div class="row">
			            		    ...
			            		    <div class="box-up">
			            		        <div class="img-info">
				             	        <div class="info-inner">
			             	         	...
			             	         	/>
			             	         	<div class="a-size">
				             	        	<select class="form-control validate " id="stcpdrate'.$row["stc_groceries_product_id"].'" style="color:black;font-weight:bold;text-align:center">'.$purchase_rate.'
				             	        	</select>
				             	            <input 
					             	          	type="number" 
					             	          	name="stcpdprice" 
		             				...
		             				<div class="box-down">
		             					...
		             					<a class="cart add_to_silent_sale_cart" href="#" id="'.$row["stc_groceries_product_id"].'">
		             					<span class="price"></span>
		             					<span class="add-to-cart">
		             						<span class="txt">Add Item</span>
		             					</span>
	...
	// go data to groceries sale table
	public function sale_process($sale_date, $customer_name, $ship_address, $order_on, $delever_on, $order_by, $delever_through, $remarks){
		...
		$date=date("Y-m-d H:i:s");
		$lokiinsertsale=mysqli_query($this->stc_dbs, "
			INSERT INTO `stc_groceries_sale`(
				`stc_groceries_sale_date`,
				`stc_groceries_sale_cust_id`,
				`stc_groceries_sale_cust_address`,
				`stc_groceries_sale_order_date`,
				`stc_groceries_sale_delevered_date`,
				`stc_groceries_sale_order_by`,
				`stc_groceries_sale_delever_through`,
				`stc_groceries_sale_status`,
				`stc_groceries_sale_remarks`,
				`stc_groceries_sale_created_by`
			) VALUES ( 
		...
		if($lokiinsertsale){
			$loki_get_max_id=mysqli_query($this->stc_dbs, "
				SELECT * FROM `stc_groceries_sale` ORDER BY `stc_groceries_sale`.`stc_groceries_sale_id` DESC
			");
			$stc_purchase_product_items_order_id=mysqli_fetch_assoc($loki_get_max_id);
			$get_sale_product_id=$stc_purchase_product_items_order_id['stc_groceries_sale_id'];
			if(empty($_SESSION['stc_silent_sale_sess'])){
				$odin = "Do not leave Items on Sale Order!!!";
			}else{
				foreach ($_SESSION["stc_silent_sale_sess"] as $key=>$value) {
			    	$friday_loki=mysqli_query($this->stc_dbs, "
			    		INSERT INTO `stc_groceries_sale_items`(
							`stc_groceries_sale_items_sale_id`,
							`stc_groceries_sale_items_item_id`,
	...
	// stc search groceries sale
	public function stc_filter_sale($search, $begdate, $endDate){
		$postatus='';
		$grnopt='';
		$check_loki=mysqli_query($this->stc_dbs, "
			SELECT * FROM `stc_groceries_sale` 
			INNER JOIN `stc_groceries_customer` 
			ON `stc_groceries_customer_id`=`stc_groceries_sale_cust_id` 
			INNER JOIN `stc_groceries_user` 
			ON `stc_groceries_user_id`=`stc_groceries_sale_created_by` 
			WHERE (
				`stc_groceries_customer_title` REGEXP '".mysqli_real_escape_string($this->stc_dbs, $search)."'
				OR `stc_groceries_sale_id` REGEXP '".mysqli_real_escape_string($this->stc_dbs, $search)."'
				OR `stc_groceries_customer_cont_no` REGEXP '".mysqli_real_escape_string($this->stc_dbs, $search)."'
				OR `stc_groceries_sale_order_by` REGEXP '".mysqli_real_escape_string($this->stc_dbs, $search)."'
				OR `stc_groceries_sale_remarks` REGEXP '".mysqli_real_escape_string($this->stc_dbs, $search)."'
			) AND (
                DATE(`stc_groceries_sale_date`)
                 BETWEEN '".mysqli_real_escape_string($this->stc_dbs, $begdate)."'
                 AND '".mysqli_real_escape_string($this->stc_dbs, $endDate)."'
            )
			ORDER BY `stc_groceries_sale`.`stc_groceries_sale_id` 
			DESC LIMIT 0,20
		");
		$odin='
			<table class="table table-hover table-bordered ">
		...
		if($do_action == 0){
			...
			foreach ($check_loki as $row) {
				$check_loki_nest=mysqli_query($this->stc_dbs, "
					SELECT * FROM `stc_groceries_sale_items` 
					WHERE `stc_groceries_sale_items_sale_id`='".$row["stc_groceries_sale_id"]."'
				");
				$soamount=0;
				$sogstamount=0;
				foreach ($check_loki_nest as $row2nd) {
					$amt = $row2nd['stc_groceries_sale_items_item_qty'] * $row2nd['stc_groceries_sale_items_item_price'];
					$gst = $amt * $row2nd['stc_groceries_sale_items_item_gst']/100;
					$soamount += $amt;
					$sogstamount += $gst;
				}
				$str_length = 5;
              	// hardcoded left padding if number < $str_length
              	$salenumber = substr("0000{$row["stc_groceries_sale_id"]}", -$str_length);
              	$total= $soamount + $sogstamount;
              	...
              	foreach($sale_payqry as $sale_payrow){
              		$paid_amt+=$sale_payrow['stc_groceries_sale_payment_value'];
              	}
				...
				$odin.='
					<tr>
       					<td>
       						'.date('d-m-y', strtotime($row["stc_groceries_sale_date"])).'
       						<br>
       						STC/T/S/'.$salenumber.'
       					</td>
       					<td class="text-center">
       						'.$row["stc_groceries_customer_title"].'
       					</td>
       					<td class="text-center">
       						'.$row["stc_groceries_customer_address"].'
       					</td>
       					<td class="text-center">
       						'.$row["stc_groceries_customer_cont_no"].'
       					</td>
       					<td>
       						'.date('d-m-y', strtotime($row["stc_groceries_sale_order_date"])).'
       						<br>
       						'.$row['stc_groceries_sale_order_by'].'
       					</td>
       					<td>
       						'.date('d-m-y', strtotime($row["stc_groceries_sale_delevered_date"])).'
       						<br>
       						'.$row['stc_groceries_sale_delever_through'].'
       					</td>
       					<td class="text-right">
       						'.number_format($total, 2).'
       					</td>
       					...
       					<td>
       						'.$row["stc_groceries_sale_remarks"].'
       					</td>
       					<td>
       						'.$row["stc_groceries_sale_status"].'
       					</td>
       					<td class="text-center">
       						'.strtoupper($row["stc_groceries_user_name"]).'
       					</td>
       					<td class="text-center">
       						...
       						<a 
       							...
       							id="'.$row["stc_groceries_sale_id"].'" 
       							href="silent-invoice-print-preview.php?pid='.$row["stc_groceries_sale_id"].'" 
       							style="font-size: 25px;color: black;"
       						><i class="fa fa-print" aria-hidden="true"></i>
       						</a>
       						<a class="stc_print_payment" id="'.$row["stc_groceries_sale_id"].'" pay-val="'.$total.'" href="#" style="font-size: 25px;color: black;">
       							Pay
       						</a> 
       					</td>
            		</tr>         	
	...
	// stc search groceries sale by date
	...
	public function stc_sale_call_for_edit($sale_id){
		...
		$odincall_saleqry=mysqli_query($this->stc_dbs, "
			...
			WHERE
				`stc_groceries_sale_id` = '".mysqli_real_escape_string($this->stc_dbs, $sale_id)."'
			ORDER BY `stc_groceries_sale_id` DESC
			LIMIT 0,1
		");
		...
		foreach($odincall_saleqry as $odincall_salerow){
			$odin_callcustqry=mysqli_query($this->stc_dbs, "
				SELECT 
					`stc_groceries_customer_id`,
					`stc_groceries_customer_title`
				FROM `stc_groceries_customer` 
				ORDER BY `stc_groceries_customer_title` ASC
			");
			$cust_name='';
			foreach($odin_callcustqry as $odin_callcustrow){
				if($odin_callcustrow['stc_groceries_customer_id']==$odincall_salerow['stc_groceries_customer_id']){
					$cust_name.='<option value="'.$odincall_salerow['stc_groceries_customer_id'].'" selected>'.$odincall_salerow['stc_groceries_customer_title'].'</option>';
				}else{
					$cust_name.='<option value="'.$odincall_salerow['stc_groceries_customer_id'].'">'.$odin_callcustrow['stc_groceries_customer_title'].'</option>';
				}
			}
			$sale_cust_name=$cust_name;
			$sale_cust_addr=$odincall_salerow['stc_groceries_sale_cust_address'];
			$sale_order_by=$odincall_salerow['stc_groceries_sale_order_by'];
			$sale_deliver_through=$odincall_salerow['stc_groceries_sale_delever_through'];
			$sale_remarks=$odincall_salerow['stc_groceries_sale_remarks'];
		}


		$odincall_saleitemqry=mysqli_query($this->stc_dbs, "
			SELECT
				`stc_groceries_sale_items_id`,
				`stc_groceries_sale_items_sale_id`,
				`stc_groceries_product_id`,
				`stc_groceries_product_title`,
				`stc_groceries_sale_items_item_qty`,
			...
			INNER JOIN 
				`stc_groceries_product` 
			ON 
				`stc_groceries_product_id`=`stc_groceries_sale_items_item_id` 
			WHERE
				`stc_groceries_sale_items_sale_id` = '".mysqli_real_escape_string($this->stc_dbs, $sale_id)."'
			ORDER BY `stc_groceries_product_title` DESC
		");
		...
		$slno=0;
		foreach($odincall_saleitemqry as $values) {  
			$amount=$values["stc_groceries_sale_items_item_price"];
			$gstamount = $amount * ($values["stc_groceries_sale_items_item_gst"]/100);
			$slno++;
			$price=$amount + $gstamount;
			...
			$sale_items .= ' 
				<tr>
					<td align="center">'.$slno.'.</td>
					<td>'.$values['stc_groceries_product_title'].'</td>
					<td>'.$values["stc_groceries_sale_items_item_unit"].'</td>
					<td>
					  <input
					  	style="padding: 5px;height: 40px;"
					    type="text"
					    id="stc-trading-item-qtysc'.$values["stc_groceries_sale_items_id"].'"
					    class="validate"
					    placeholder="Quantity"
					    value="'.number_format($values["stc_groceries_sale_items_item_qty"], 2, '.', '').'"
					  />
					</td>
					<td class="text-right">
					  <input
					  	style="padding: 5px;height: 40px;"
					    type="text"
					    id="stc-trading-item-ratesc'.$values["stc_groceries_sale_items_id"].'"
					    class="validate"
					    placeholder="Quantity"
					    value="'.number_format($price, 2, '.', '').'"
					  />
					  <input type="hidden" id="stc-trading-item-gstsc'.$values["stc_groceries_sale_items_id"].'" value="'.$values["stc_groceries_sale_items_item_gst"].'">
					</td>
					<td align="right"><i class="fas fa-rupee-sign"></i> '.number_format($price * $values['stc_groceries_sale_items_item_qty'], 2).'</td>
					<td align="center">
						<a href="#" class="btn btn-primary stc-customer-update-item-edit" id="'.$values["stc_groceries_sale_items_id"].'"><i class="fa fa-check-circle"></i></a>
						<a href="#" class="btn btn-danger stc-customer-remove-edit" id="'.$values["stc_groceries_sale_items_id"].'"><i class="fa fa-trash"></i></a>
					</td>
				</tr>
			';  
			$total += $values["stc_groceries_sale_items_item_qty"] * $price;  
			$totalgst +=$gstamount;
		}  
		$sale_items .= ' 
     		<tr>
	...
	// call sale item
	public function stc_groceries_sale_item_call($sale_id){
		$odin='';
		$odincall_saleitemqry=mysqli_query($this->stc_dbs, "
			SELECT
				`stc_groceries_sale_items_id`,
	...
	public function stc_groceries_sale_item_add($sale_id, $item_id, $item_unit, $item_qty, $item_rate){
		...
		$odin_checkqry=mysqli_query($this->stc_dbs, "
			...
			FROM 
				`stc_groceries_sale_items` 
			WHERE 
				`stc_groceries_sale_items_item_id`='".mysqli_real_escape_string($this->stc_dbs, $item_id)."' 
			AND 
				`stc_groceries_sale_items_sale_id`='".mysqli_real_escape_string($this->stc_dbs, $sale_id)."' 
		");
		if(mysqli_num_rows($odin_checkqry)>0){
	...
	// update items
	public function stc_groceries_sale_item_update($item_id, $item_qty, $item_rate, $item_gst){
		$odin='';
		$odin_checkqry=mysqli_query($this->stc_dbs, "
			SELECT 
				`stc_groceries_sale_items_id` 
			FROM 
				`stc_groceries_sale_items` 
			WHERE 
				`stc_groceries_sale_items_id` = '".mysqli_real_escape_string($this->stc_dbs, $item_id)."'
		");
		if(mysqli_num_rows($odin_checkqry)>0){
			$basic=$item_rate;
			$gstamount = $basic-($basic*(100/(100+$item_gst)));
			...
			$odin_updateqry=mysqli_query($this->stc_dbs, "
				UPDATE
				    `stc_groceries_sale_items`
				SET
					`stc_groceries_sale_items_item_qty` = '".mysqli_real_escape_string($this->stc_dbs, $item_qty)."',
					`stc_groceries_sale_items_item_price` = '".mysqli_real_escape_string($this->stc_dbs, $saleprice)."'
				WHERE
				    `stc_groceries_sale_items_id` = '".mysqli_real_escape_string($this->stc_dbs, $item_id)."'
			");
			if($odin_updateqry){
	...
	// remove items
	public function stc_groceries_sale_item_remove($item_id){
		$odin='';
		$odin_removeqry=mysqli_query($this->stc_dbs, "
			DELETE FROM `stc_groceries_sale_items` WHERE `stc_groceries_sale_items_id` = '".mysqli_real_escape_string($this->stc_dbs, $item_id)."'
		");
		if($odin_removeqry){
			$odin="Item Removed Successfully.";
		}else{
	...
	// update edited sale
	public function stc_groceries_sale_edit_update($sale_id, $cust_id, $cust_addresss, $cust_order_by, $cust_del_through, $cust_remarks){
		$odin='';
		$odin_updteqry=mysqli_query($this->stc_dbs, "
			UPDATE
				`stc_groceries_sale`
			SET
				`stc_groceries_sale_cust_id` = '".mysqli_real_escape_string($this->stc_dbs, $cust_id)."',
				`stc_groceries_sale_cust_address` = '".mysqli_real_escape_string($this->stc_dbs, $cust_addresss)."',
				`stc_groceries_sale_order_by` = '".mysqli_real_escape_string($this->stc_dbs, $cust_order_by)."',
				`stc_groceries_sale_delever_through` = '".mysqli_real_escape_string($this->stc_dbs, $cust_del_through)."',
				`stc_groceries_sale_remarks` = '".mysqli_real_escape_string($this->stc_dbs, $cust_remarks)."'
			WHERE
				`stc_groceries_sale_id` = '".mysqli_real_escape_string($this->stc_dbs, $sale_id)."'
		");
		if($odin_updteqry){
	...
	public function stc_sale_payment_set($sale_id, $payment_type, $payment_amount, $payment_remarks, $check_amount){
		...
		$odin_checkqry=mysqli_query($this->stc_dbs, "
			...
			FROM
				`stc_groceries_sale_payment`
			WHERE 
				`stc_groceries_sale_payment_sale_id` = '".mysqli_real_escape_string($this->stc_dbs, $sale_id)."'
		");
		foreach($odin_checkqry as $odin_checkrow){
			$paid_amount+=$odin_checkrow['stc_groceries_sale_payment_value'];
		}
		$due_amount=$check_amount - $paid_amount;
		if($payment_amount>$due_amount){
			$odin='Invalid Amount.';
			...
			$odin_paycheckqry=mysqli_query($this->stc_dbs, "
				INSERT INTO `stc_groceries_sale_payment`(
					`stc_groceries_sale_payment_date`,
					`stc_groceries_sale_payment_sale_id`,
					`stc_groceries_sale_payment_type`,
					`stc_groceries_sale_payment_value`,
					`stc_groceries_sale_payment_remarks`,
					`stc_groceries_sale_payment_created_by`
				) VALUES (
					'".mysqli_real_escape_string($this->stc_dbs, $date)."',
					'".mysqli_real_escape_string($this->stc_dbs, $sale_id)."',
					'".mysqli_real_escape_string($this->stc_dbs, $payment_type)."',
					'".mysqli_real_escape_string($this->stc_dbs, $payment_amount)."',
					'".mysqli_real_escape_string($this->stc_dbs, $payment_remarks)."',
					'".mysqli_real_escape_string($this->stc_dbs, $_SESSION['stc_groceries_user_id'])."'
				)
			");
			if($odin_paycheckqry){
				$odin='Payment Done!!!';
...
if(isset($_POST['stc_show_purchase_sess'])){
	...
	$slno = '';  
	$order_table .= '  
		<table class="table table-hover table-bordered table-responsive" align="centre">
			<thead>
			  <tr>
...
if(isset($_POST['stc_trading_edit_padd_item'])){
	...
	$objloki=new aetherPurchase();
	$objlokiout=$objloki->stc_groceries_sale_item_add($sale_id, $item_id, $item_unit, $item_qty, $item_rate);
	echo $objlokiout;
}
...
if(isset($_POST['stc_trading_edit_pupdate'])){
	...
	$objloki=new aetherPurchase();
	$objlokiout=$objloki->stc_groceries_purchase_item_update($item_id, $item_qty, $item_rate, $item_gst);
	echo $objlokiout;
}
...
if(isset($_POST['stc_trading_edit_premove'])){
	$item_id=$_POST['item_id'];
	$objloki=new aetherPurchase();
	$objlokiout=$objloki->stc_groceries_purchase_item_remove($item_id);
	echo $objlokiout;
}
#<---------------------------------Silent Sale Section----------------------------------->
...
if(isset($_POST['stc_trading_edit_call_item'])){
	$sale_id=$_POST['sale_id'];
	$objloki=new aetherSale();
	$objlokiout=$objloki->stc_groceries_sale_item_call($sale_id);
	echo $objlokiout;
}
...
if(isset($_POST['stc_trading_edit_update'])){
	...
	$objloki=new aetherSale();
	$objlokiout=$objloki->stc_groceries_sale_item_update($item_id, $item_qty, $item_rate, $item_gst);
	echo $objlokiout;
}
...
if(isset($_POST['stc_trading_edit_remove'])){
	$item_id=$_POST['item_id'];
	$objloki=new aetherSale();
	$objlokiout=$objloki->stc_groceries_sale_item_remove($item_id);
	echo $objlokiout;
}
...
if(isset($_POST['stc_trading_sale_save_edit'])){
	...
	$objloki=new aetherSale();
	$objlokiout=$objloki->stc_groceries_sale_edit_update($sale_id, $cust_id, $cust_addresss, $cust_order_by, $cust_del_through, $cust_remarks);
	echo $objlokiout;	
}
