	...
	public function stc_call_expense(){
		...
		$odin_get_req_qry=mysqli_query($this->stc_dbs, "
			...
			INNER JOIN 
				`stc_groceries_user` 
			ON
				`stc_groceries_user_id`=`stc_expenses_created_by`
			WHERE 
				`stc_expenses_created_by`='".$_SESSION['stc_groceries_user_id']."' 
			AND 
		...
		if(mysqli_num_rows($odin_get_req_qry)>0){
			foreach($odin_get_req_qry as $req_row){
				$odin.='
					<tr>
						<td class="text-right">STC/G/E/'.$req_row['stc_expenses_id'].'</td>
						...
						<td class="text-center">'.$req_row['stc_groceries_user_name'].'</td>
					</tr>
				';
			}
			$odin.='
