...
class aetherLedger extends tesseract{	
	...
	public function stc_call_ledger($cust_id){
		$odin_getqry=mysqli_query($this->stc_dbs, "
			...
			ON 
				`stc_groceries_customer_id`=`stc_groceries_sale_cust_id`
			WHERE
	    ...
	    if(mysqli_num_rows($odin_getqry)>0){
	    	foreach($odin_getqry as $odin_getqryrow){
	    		...
	    		$odin_getitemqry=mysqli_query($this->stc_dbs, "
					...
					ON `stc_groceries_sale_items_item_id`=`stc_groceries_product_id`
					WHERE
						`stc_groceries_sale_items_sale_id` = '".mysqli_real_escape_string($this->stc_dbs, $odin_getqryrow['stc_groceries_sale_id'])."'
					ORDER BY `stc_groceries_product_title` ASC
				");
				foreach($odin_getitemqry as $odin_getitemrow){
		    		$basic=$odin_getitemrow['stc_groceries_sale_items_item_qty'] * $odin_getitemrow['stc_groceries_sale_items_item_price'];
		    		$gst=$basic * $odin_getitemrow['stc_groceries_sale_items_item_gst']/100;
		    		$total+=$basic + $gst;
		    	}
	    		$odin.='
	    			<tr>
	    				<td class="text-center">STC/T/S/'.$odin_getqryrow['stc_groceries_sale_id'].'</td>
	    				<td class="text-center">'.$odin_getqryrow['stc_groceries_sale_date'].'</td>
	    				<td class="text-center">'.$odin_getqryrow['stc_groceries_customer_title'].'</td>
	    				<td class="text-center">'.$odin_getqryrow['stc_groceries_customer_cont_no'].'</td>
	    		...
	    		$odin_getpayqry=mysqli_query($this->stc_dbs, "
					...
					WHERE
						`stc_groceries_sale_payment_sale_id` = '".mysqli_real_escape_string($this->stc_dbs, $odin_getqryrow['stc_groceries_sale_id'])."'
				");
				foreach($odin_getpayqry as $odin_getpayrow){
		    		$totalpaid+=$odin_getpayrow['stc_groceries_sale_payment_value'];
		    	}
	    		$odin.='
	    			<tr>
