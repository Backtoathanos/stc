...
class aetherCustomer extends tesseract{
	...
	public function customer_process($silentcustomername, $silentcustomeraddress, $silentcustomercontactno, $silentcustomerpincode, $silentcustomercity, $silentcustomerstate, $silentcustomertype, $silentcustomerremarks){
		$odin='';
		$lokicheckqry=mysqli_query($this->stc_dbs, "
			...
			WHERE
			    `stc_groceries_customer_title` = '".mysqli_real_escape_string($this->stc_dbs, $silentcustomername)."' OR 
			    `stc_groceries_customer_cont_no` = '".mysqli_real_escape_string($this->stc_dbs, $silentcustomercontactno)."'
		");
		if(mysqli_num_rows($lokicheckqry)>0){
			...
			$lokiinsertcustomer=mysqli_query($this->stc_dbs, "
				INSERT INTO `stc_groceries_customer`(
					`stc_groceries_customer_title`,
					`stc_groceries_customer_address`,
					`stc_groceries_customer_cont_no`,
					`stc_groceries_customer_pincode`,
					`stc_groceries_customer_city_id`,
					`stc_groceries_customer_state_id`,
					`stc_groceries_customer_type`,
					`stc_groceries_customer_remarks`,
					`stc_groceries_customer_createdby`
				) VALUES (
					...
					'".mysqli_real_escape_string($this->stc_dbs, $silentcustomertype)."',
	...
	public function stc_call_customer($search){
		$check_loki=mysqli_query($this->stc_dbs, "
			...
			FROM
				`stc_groceries_customer`
			INNER JOIN `stc_city` 
			ON `stc_city_id` = `stc_groceries_customer_city_id`
			INNER JOIN `stc_state` 
			ON `stc_state_id` = `stc_groceries_customer_state_id`
			INNER JOIN `stc_groceries_user` 
			ON `stc_groceries_user_created_by` = `stc_groceries_customer_createdby`
			WHERE 
				`stc_groceries_customer_title` 	REGEXP 	'".mysqli_real_escape_string($this->stc_dbs, $search)."' OR
				`stc_groceries_customer_address` 	REGEXP 	'".mysqli_real_escape_string($this->stc_dbs, $search)."' OR
				`stc_groceries_customer_cont_no` 	REGEXP 	'".mysqli_real_escape_string($this->stc_dbs, $search)."' OR
				`stc_groceries_customer_pincode` 	REGEXP 	'".mysqli_real_escape_string($this->stc_dbs, $search)."' OR
				`stc_groceries_customer_remarks` 	REGEXP 	'".mysqli_real_escape_string($this->stc_dbs, $search)."' OR
		...
		if($do_action == 0){
			...
			foreach ($check_loki as $row) {
				$cust_type='';
				if($row["stc_groceries_customer_type"]==1){
					$cust_type='Retailer';
				}elseif($row["stc_groceries_customer_type"]==2){
					$cust_type='Contractor';
				}elseif($row["stc_groceries_customer_type"]==3){
					$cust_type='Walk in';
				}elseif($row["stc_groceries_customer_type"]==4){
					$cust_type='Defaulter';
				}else{
					$cust_type='Refered';
				}
				$odin.='
					<tr>
       					<td class="text-center">
       					  <p>'.$row["stc_groceries_customer_title"].'</p>
       					</td>
       					<td class="text-left">
       					  <p>'.$row["stc_groceries_customer_address"].'</p>
       					</td>
       					<td class="text-center">
       					  '.$row["stc_city_name"].'<br>
       					  '.$row["stc_state_name"].'<br>
       					  '.$row["stc_groceries_customer_pincode"].'
       					</td>
       					<td class="text-center">
       					  <p>'.$row["stc_groceries_customer_cont_no"].'</p>
       					</td>
       					<td class="text-center">
       					  <p>'.$cust_type.'</p>
       					</td>
       					<td class="text-center">
       					  <p>'.$row["stc_groceries_customer_remarks"].'</p>
       					</td>
       					<td class="text-center">
       					...
       					<td class="text-center">
       					  <a 
       					  	...
       					  	id="'.$row['stc_groceries_customer_id'].'"
	...
	public function stc_call_customer_for_edit($cust_id){
		...
		$odincustqry=mysqli_query($this->stc_dbs, "			
			...
			FROM
				`stc_groceries_customer`
			WHERE 
				`stc_groceries_customer_id` = '".mysqli_real_escape_string($this->stc_dbs, $cust_id)."'
			ORDER BY `stc_groceries_customer_id` DESC LIMIT 0,1
		");

		foreach ($odincustqry as $value) {			
			$title = $value['stc_groceries_customer_title'];
			$address = $value['stc_groceries_customer_address'];
			$cont_no = $value['stc_groceries_customer_cont_no'];
		}
	...
	public function stc_save_customer_edit($cust_id, $cust_title, $cust_address, $cust_cont_no){
		...
		$odineditcustqry=mysqli_query($this->stc_dbs, "
			UPDATE
			    `stc_groceries_customer`
			SET
				`stc_groceries_customer_address`='".mysqli_real_escape_string($this->stc_dbs, $cust_address)."',
			    `stc_groceries_customer_cont_no`='".mysqli_real_escape_string($this->stc_dbs, $cust_cont_no)."'
			WHERE
			    `stc_groceries_customer_id`='".mysqli_real_escape_string($this->stc_dbs, $cust_id)."'
		");
		if($odineditcustqry){
			$odin='Customer Edited Successfully!!!';
		}else{
			$odin='Hmmmm!!! Something Went Wrong Customer Not Edited Successfully. Please Check & Try Again Later.';
		}
		return $odin;
	}
}
...
if(isset($_POST['save_customer_action'])){
	...
	$silentcustomertype 		= $_POST['silentcustomertype'];
	...
	if(empty($silentcustomername) || empty($silentcustomeraddress) || empty($silentcustomercontactno)){
		...
		$fridaycustomer=$fridaycustomer->customer_process($silentcustomername, $silentcustomeraddress, $silentcustomercontactno, $silentcustomerpincode, $silentcustomercity, $silentcustomerstate, $silentcustomertype, $silentcustomerremarks);
		$out=$fridaycustomer;
	}
	echo $out;
}
