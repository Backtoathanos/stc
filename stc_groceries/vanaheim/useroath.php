...
class dante extends tesseract{	
	...
	public function stc_login($user,$pass){
		$checkbumblebee=mysqli_query($this->stc_dbs, "
			SELECT `stc_groceries_user_id`,`stc_groceries_user_name` FROM `stc_groceries_user` 
			WHERE (`stc_groceries_user_cont`='".$user."'
				OR `stc_groceries_user_email`='".$user."'
			) 
			AND `stc_groceries_user_password`='".$pass."'
			AND `stc_groceries_user_is_user`=1
		");
