...
class prime extends tesseract{
	...
	public function stc_ag_order_on_agent(){
		...
		$optimusprimequery=mysqli_query($this->stc_dbs, "
			...
			WHERE `stc_own_agent_order_agent_id`='".$_SESSION['stc_agent_id']."'
		...
		if($countopquery>0){
			foreach($optimusprimequery as $primerow){
				...
				$nestedoptimusrime=mysqli_query($this->stc_dbs, "
					...
					WHERE `stc_own_agent_order_items_order_id`='".$primerow['stc_own_agent_order_id']."'
				");
				foreach($nestedoptimusrime as $getprice){
					$optotal+=$getprice['stc_own_agent_order_items_pd_qty'] * $getprice['stc_own_agent_order_items_pd_price'];
				}
				$optimusprime.="
					<tr>
						<td>STC/A/".substr("0000{$primerow['stc_own_agent_order_id']}", -5)."<br>".date('d-M-Y', strtotime($primerow['stc_own_agent_order_date']))."</td>
						<td align='right'><i class='fas fa-rupee'></i> ".number_format($optotal, 2)."</td>
						<td>".$primerow['stc_own_agent_order_status']."</td>
						<td><a href='#' style='font-size: 25px;font-weight: bold;color: black;' class='ag-show-grid' id='".$primerow['stc_own_agent_order_id']."'><i class='fas fa-eye'></i></a></td>
					</tr>
					<tr>
						<td colspan='4'>
							<div style='display:none;' id='togdiv".$primerow['stc_own_agent_order_id']."'>
								Loading...
							</div>
						</td>
					</tr>
	...
	public function stc_ag_order_items_call($odid){
		...
		$transformersquery=mysqli_query($this->stc_dbs, "
			...
			INNER JOIN `stc_own_agent_order`
			ON `stc_own_agent_order_items_order_id`=`stc_own_agent_order_id`
			INNER JOIN `stc_sub_category`
			ON `stc_product_sub_cat_id`=`stc_sub_cat_id`
			WHERE `stc_own_agent_order_items_order_id`='".$odid."'
		");
		if(mysqli_num_rows($transformersquery)>0){
			foreach($transformersquery as $firstrow){
				$getrecivedtransformers=mysqli_query($this->stc_dbs, "
					...
					WHERE `stc_sale_product_agent_id`='".$firstrow['stc_own_agent_order_agent_id']."'	
				...
				$stringpd='';
				if(strlen($firstrow['stc_product_name'])>15){
					$stringpd=substr($firstrow['stc_product_name'], 0, 15);
					$stringpd=$stringpd.'...';
				}
				$transformers.='
					<tr>
						<td>
							<p>'.$subcat.' '.$stringpd.'</p>
							<b>'.$firstrow['stc_own_agent_order_items_pd_price'].'/'.$firstrow['stc_product_unit'].'</b>
						</td>
						<td>'.number_format($firstrow['stc_own_agent_order_items_pd_qty'], 2).'</td>
						<td><i class="fas fa-rupee"></i> '.number_format($firstrow['stc_own_agent_order_items_pd_price'] * $firstrow['stc_own_agent_order_items_pd_qty'], 2).'</td>
					</tr>
				';
				$sl++;
				$countqty=0;
			...
			$transformers.='
					<tr>
						<td colspan="5"><h4>No Products Found!!!</h6></td>
					</tr>
			';
		}
		$transformers.='
